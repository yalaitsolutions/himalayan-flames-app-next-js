...
import {
  getMenu, adminLogin, adminMe, saveMenu, uploadImage, getToken, setToken,
} from "../api.js";
  ...
  return (
    <div className="admin-login-wrap">
      <form className="admin-login" onSubmit={submit}>
        ...
        <h1>Owner <span className="gold">Login</span></h1>
        <p>Himalayan Flames — Menu Manager</p>
        {err && <div className="admin-toast err" style={{ marginBottom: "1rem", borderRadius: 8 }}>{err}</div>}
        <div className="admin-field">
          <label htmlFor="al-u">Username</label>
          <input id="al-u" autoComplete="username" value={u} onChange={(e) => setU(e.target.value)} required />
        </div>
        <div className="admin-field" style={{ marginTop: ".75rem" }}>
          <label htmlFor="al-p">Password</label>
          <input id="al-p" type="password" autoComplete="current-password" value={p} onChange={(e) => setP(e.target.value)} required />
        </div>
...
function ImageField({ currentSrc, onFileSelected, onUrlChange }) {
  ...
  // keep the url draft in sync when a different item is opened for editing
  useEffect(() => {
...
// ============================================================
//  CATEGORY SETTINGS  (collapsible edit form for category meta)
// ============================================================
function CategorySettings({ section, onUpdate, onDelete }) {
  const [open, setOpen] = useState(false);
  const [draft, setDraft] = useState({ ...section });

  // Reset draft when a different category is selected
...
// ============================================================
//  ITEM PANEL  (right column — items for the active category)
// ============================================================
function ItemPanel({
...
function Dashboard({ user, onLogout }) {
  ...
  // Mark unsaved when sections change (but not on the initial load)
  useEffect(() => {
  ...
  useEffect(() => {
    getMenu()
      .then((d) => {
        firstLoad.current = true; // next sections change is from load, not an edit
...
// ============================================================
//  AUTH GATE  (checks stored token, shows login or dashboard)
// ============================================================
