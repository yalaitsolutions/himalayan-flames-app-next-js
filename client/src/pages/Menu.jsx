...
export default function Menu() {
  ...
  useEffect(() => {
    getMenu()
      .then((d) => {
        ...
        if (d.sections.length) setActive(d.sections[0].id);
      })
      .catch(() => {});
  }, []);

  usePageEffects([sections.length]);
  ...
  const showPopup = useCallback((item, e) => {
    ...
    // Fire blazes in then dissolves after 1 s
    if (item.spicy) {
  ...
  return (
    <>
      ...
      >
        <div id="popupImgWrap">
          <img id="popupImg" src={popup.img} alt={popup.name} />
